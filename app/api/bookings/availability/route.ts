...
import { isNotNull, sql, gt } from "drizzle-orm";
...
export async function GET() {
  try {
    ...
    // Count confirmed bookings per date
    const bookingRows = await db
      ...
      .from(bookings)
      .where(isNotNull(bookings.bookingDate))
      .groupBy(bookings.bookingDate);

    // Count active (non-expired) reservations per date
    const reservationRows = await db
      ...
      .where(gt(slotReservations.expiresAt, now))
    ...
    return NextResponse.json(
      { fullyBookedDates: [], dateCounts: {}, maxPerDay: MAX_BOOKINGS_PER_DAY },
      { status: 200 }
    );
  }
}
